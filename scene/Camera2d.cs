...
public partial class Camera2d : Camera2D
{
    ...
    private Vector2 _preciousMousePosition;
    private Vector2 _newZoom;    //  这两个“new”字段用于存储缩放和平移的目标值，来实现平滑过渡效果（用Lerp方法）
    private Vector2 _newPosition;
    private Viewport _viwport;
    private Vector2 _pViewportSize;
    ...
    public override void _Ready()
    {
        ...
        _viwport = GetViewport();
        _pViewportSize = _viwport.GetVisibleRect().Size;

        float viewportSizeX = _pViewportSize.X;
        float viewportSizeY = _pViewportSize.Y;
        _cameraSizeX = viewportSizeX / 2.0f / Zoom.X;
    ...
    public override void _Process(double delta)
    {
        ...
        Vector2 viewportSize = _viwport.GetVisibleRect().Size;

        //  如果视口大小发生变化或者缩放发生变化，则重新计算摄像机的大小（计算要用的理论值）
        if (viewportSize != _pViewportSize || Zoom != _newZoom)
        {
            ...
            _pViewportSize = viewportSize;
        }


        //  这段代码实现了中建平移摄像机的功能，原理是获取鼠标当前位置和上一次鼠标位置的差值，然后根据这个差值来调整摄像机的位置，从而实现平移效果
        Vector2 currentMousePosition = GetGlobalMousePosition();
        float deltaX = currentMousePosition.X - _preciousMousePosition.X;
        float deltaY = currentMousePosition.Y - _preciousMousePosition.Y;
        ...
        {
            _preciousMousePosition = currentMousePosition;
        }
    ...
    public override void _Input(InputEvent @event)
    {
        ...
        if (@event is InputEventMouseButton mouseEvent)
        {
            ...
            else if (mouseEvent.ButtonIndex == MouseButton.WheelDown && mouseEvent.Pressed)
            {
                ...
                _newZoom = Zoom * (1.0f - ZoomSpeed); // 缩小
            }
        }
    ...
    {
        //  记住相对位置是上负下正
        float cameraLeftEdge = Position.X - _cameraSizeX;
        float cameraRightEdge = Position.X + _cameraSizeX;
        float cameraTopEdge = Position.Y - _cameraSizeY;
        float cameraDownEdge = Position.Y + _cameraSizeY;
        ...
        {
            _newPosition -= new Vector2(EdgePanSpeed / Zoom.X, 0);
        }
        ...
        {
            _newPosition -= new Vector2(0, EdgePanSpeed / Zoom.X);
        }
